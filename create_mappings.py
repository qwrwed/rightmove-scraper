...
def do_stuff(input_dir: Path, output_dir: Path):
    for filepath in input_dir.glob("*.json"):
        mappings_single, mappings_multiple = do_file(filepath)
        if mappings_single:
            outfile = Path(
                output_dir, filepath.with_stem(f"{filepath.stem}-mappings_single").name
            )
            dump_data(mappings_single, outfile)
        if mappings_multiple:
            outfile = Path(
                output_dir,
                filepath.with_stem(f"{filepath.stem}-mappings_multiple").name,
            )
...
def main():
    args = parse_args()
    do_stuff(args.input_dir, args.output_dir)
